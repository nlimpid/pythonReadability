...
class Readability:
    ...
    def __init__(self, input, url):
        ...
        self.input = self.regexps['replaceFonts'].sub(
            "<\g<1>span>", self.input)
...
#        print self.html
    ...
    def grabArticle(self):
        # 这个循环删掉了'body'，另外如果在不太可能的 tag 里有'id'、'class'并且有可能里面没有，那么删掉这两个。
        # 如果 div 里面有一些其他的 divToP 里的元素，那么把这个 div 改为 p。
        #
        for elem in self.html.findAll(True):

            unlikelyMatchString = str(
                elem.get('id', '')) + str(elem.get('class', ''))
...
#                pass
            ...
            if elem.name == 'div':
                ...
                if not self.regexps['divToPElements'].search(s.decode()):
                    elem.name = 'p'
        # 对于每个 p 标签，找到它的父标签和祖父标签以及内容。每一个 tag 实际对应于每一个权重。后面再处理的时候对 父亲 和 祖父级别不同
...
#            print parentNode
            ...
            if grandParentNode and grandParentHash not in self.candidates:
                self.candidates[grandParentHash] = self.initializeNode(
                    grandParentNode)
...
#            print node
        ...
        # 根据链接数量对得分进行修改，并获得最高得分的候选值。然后执行 cleanArticle。
        #
...
#            print content
    ...
    def cleanArticle(self, content):
        ...
        return content
    # 如果有 object 和 embed。并且这里面还有 videos，那么放过。其他都删掉。
    ...
    # 初始化和权重来计算每个 tag 的得分。id 和 class 看做一个东西。
